...
class CLQManager
{
	public:
		...
		/*!
		 * \brief To Get ReadRingCount
		 * \param None
		 * \return Count of Read Ring
		 */
		uint8_t GetReadRingCount() { return m_unReadRingCount; }	
		/*!
		 * \brief To Get WriteRingCount
		 * \param None
		 * \return Count of Write Ring
		 */
		uint8_t GetWriteRingCount() { return m_unWriteRingCount; }	
		...
		//! Send Command
		int SendCommand(char *a_szProcName, int a_nInstanceID, char *a_pszData, int a_nSize, bool a_bSendSignal = false);
		//! Receive Command
		int ReceiveCommand(char **a_pszBuff);
		//! Send Result of Command
		int SendCommandResult(char *a_pstData, int a_nSize, bool a_bSendSignal = false);
		//##################################################//
