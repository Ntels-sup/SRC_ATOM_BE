...
RSAMsg::~RSAMsg()
{
	...
	{
		for(it = m_mapSummary.begin(); it != m_mapPerf.end() ; ++it)
		{
			if(it->second)
				delete it->second;
		}
...
int RSAMsg::SendResponseMsg(time_t a_tCur, RESOURCE *a_pstRsc, int a_nGroupCnt)
{
	...
	SendMonitoringInfo(tCur, pstTime, a_pstRsc, a_nGroupCnt);
	SendSummaryInfo(tCur, pstTime, a_pstRsc, a_nGroupCnt);
	SendPerformanceInfo(tCur, pstTime, a_pstRsc, a_nGroupCnt);
...
int RSAMsg::ProcessPeerIpCommand(CProtocol *a_pclsProto, RESOURCE *a_pstRsc, int a_nGroupCnt)
{
	...
	try {
		...
		strPkgName.assign(doc["pkg_name"].str());
		strNodeType.assign(doc["node_type"].str());
...
int RSAMsg::ProcessNicIpCommand(CProtocol *a_pclsProto, RESOURCE *a_pstRsc, int a_nGroupCnt)
{
	...
	try {
		...
		strIfName.assign(doc["nic"].str());
		strVersion.assign(doc["ip_version"].str());
