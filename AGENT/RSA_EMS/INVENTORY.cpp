...
int INVENTORY::Initialize(CFileLog *a_pclsLog, RESOURCE *a_pRsc, void *a_pclsMain)
{
	...
	if(ConnectDB() < 0)
	{
		m_pclsLog->ERROR("Failed to Connect DB");
		return -1;
	}
...
int INVENTORY::GetCPUInfo()
{
	...
	{
		m_pclsLog->ERROR("(%s) Insert CPU Info Failed [%d:%s]", szQuery, m_pclsDB->GetError(), m_pclsDB->GetErrorMsg());
		delete m_pclsDB;
		m_pclsDB = NULL;
		pclose(fp);
		return -1;
...
int INVENTORY::GetKernelInfo()
{
	...
	if(m_pclsDB->Execute(szQuery, strlen(szQuery)) < 0)
	{
		m_pclsLog->ERROR("(%s) Insert Kernel Info Failed [%d:%s]", szQuery, m_pclsDB->GetError(), m_pclsDB->GetErrorMsg());
		delete m_pclsDB;
		m_pclsDB = NULL;
		pclose(fp);
		return -1;
...
int INVENTORY::GetMemoryInfo()
{
	...
	if(m_pclsDB->Execute(szQuery, strlen(szQuery)) < 0)
	{
		m_pclsLog->ERROR("(%s) Insert MEMORY Info Failed [%d:%s]", szQuery, m_pclsDB->GetError(), m_pclsDB->GetErrorMsg());
		delete m_pclsDB;
		m_pclsDB = NULL;
		pclose(fp);
		return -1;
...
int INVENTORY::GetDBInfo()
{
	...
	if(m_pclsDB->Execute(szQuery, strlen(szQuery)) < 0)
	{
		m_pclsLog->ERROR("(%s) Insert DB Info Failed [%d:%s]", szQuery, m_pclsDB->GetError(), m_pclsDB->GetErrorMsg());
		delete m_pclsDB;
		m_pclsDB = NULL;
		return -1;
	}
...
int INVENTORY::GetOSInfo()
{
	...
	if(m_pclsDB->Execute(szQuery, strlen(szQuery)) < 0)
	{
		m_pclsLog->ERROR("(%s) Insert OS Info Failed [%d:%s]", szQuery, m_pclsDB->GetError(), m_pclsDB->GetErrorMsg());
		delete m_pclsDB;
		m_pclsDB = NULL;
		pclose(fp);
		return -1;
...
int INVENTORY::GetPythonInfo()
{
	...
	if(m_pclsDB->Execute(szQuery, strlen(szQuery)) < 0)
	{
		m_pclsLog->ERROR("(%s) Insert Python Info Failed [%d:%s]", szQuery, m_pclsDB->GetError(), m_pclsDB->GetErrorMsg());
		delete m_pclsDB;
		m_pclsDB = NULL;
		return -1;
	}
...
int INVENTORY::Run()
{
	...
	if( m_pclsDB->Execute(szBuffer, strlen(szBuffer)) < 0)
	{
		m_pclsLog->ERROR("Query (%s) Failed [%d:%s]", szBuffer, m_pclsDB->GetError(), m_pclsDB->GetErrorMsg());
		return -1;
	}
