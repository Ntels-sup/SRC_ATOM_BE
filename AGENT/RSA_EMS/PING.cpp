...
int PING::GetPeerInfo(std::vector<PEER_INFO> &a_vecPeerInfo)
{
	...
	DB *pclsDB = new (std::nothrow) MariaDB();
	if(NULL == pclsDB)
	{
	...
	snprintf(szQuery, sizeof(szQuery), "SELECT IP, IMAGE_NO FROM TAT_NODE WHERE NODE_NO='%d'", m_pclsMain->GetNodeID());
...
int PING::MakeNodeJson(const char *a_szPkgName, const char *a_szPeerType, std::string &a_strResult)
{
	...
	try{
		...
		rabbit::array a_nodeList = doc["node_list"];
