...
bool CCmdVnfStart::RequestParse(const char* a_szJson)
{
	if (a_szJson == NULL) {
		return false;
	}
    ...
    try {
		...
		g_pclsLog->ERROR("CMD 'VNF_START', %s", e.what());
		return false;
	} catch(rabbit::parse_error& e) {
		g_pclsLog->ERROR("CMD 'VNF_START', %s", e.what());
		return false;
	} catch(...) {
		g_pclsLog->ERROR("CMD 'VNF_START', unknown error");
		return false;
	}
...
string CCmdVnfStart::ResponseGen(void)
{
