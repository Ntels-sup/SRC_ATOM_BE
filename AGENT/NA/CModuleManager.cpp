...
bool CModuleManager::AddModule(string& a_strModPath)
{
	...
	strncpy(pclsMod->m_stOption.m_szNodeVersion, clsCfg.m_strVersion.c_str(),
									sizeof(pclsMod->m_stOption.m_szNodeVersion));
	...
	CMesgRoute::Instance().AddRoute(nProcNo, nProcNo);
...
bool CModuleManager::SetOptNodeVersion(string& a_strNodeVer, const char* a_szModName)
{
	if (a_szModName != NULL) {
		auto iter = m_mapModule.find(a_szModName);
		if (iter == m_mapModule.end()) {
		...
		strncpy(iter->second->m_stOption.m_szNodeVersion, a_strNodeVer.c_str(),
								sizeof(iter->second->m_stOption.m_szNodeVersion)-1);
		return true;
	}
