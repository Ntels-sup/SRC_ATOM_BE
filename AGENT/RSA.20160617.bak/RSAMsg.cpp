...
int RSAMsg::ProcessCmd(CProtocol *a_pclsProto, time_t a_tCur, RESOURCE *a_pstRsc, int a_nGroupCnt, void* a_pclsMain)
{
	...
	switch (nCmd) {
		...
		case CMD_RSA_PEER_IP_QUERY:
			ProcessPeerIpCommand(a_pclsProto, a_pstRsc, a_nGroupCnt, a_pclsMain);
			break;
...
int RSAMsg::SendInitMsgToRoot()
{
	...
	try {
		...
		m_pclsRootSock->SetSource(m_pstModuleOption->m_nNodeNo, PROCID_ATOM_NA_RSA);
		m_pclsRootSock->SetDestination(m_pstModuleOption->m_nNodeNo, PROCID_ATOM_NA_RSA);
...
int RSAMsg::SendMonitoringMsg(DST_INFO *a_pstDstInfo, const char* a_szPayload)
{
	...
	clsResp.SetSource(m_pstModuleOption->m_nNodeNo, PROCID_ATOM_NA_RSA);
...
int RSAMsg::ProcessPeerIpCommand(CProtocol *a_pclsProto, RESOURCE *a_pstRsc, int a_nGroupCnt, void* a_pclsMain)
{
	...
	for(int i = 0; i < a_nGroupCnt ; i++)
	{
		...
		// cjr _ 20160610
		pclsRSA->Initialize( m_pclsLog, &(a_pstRsc[i]), a_pclsMain); 
