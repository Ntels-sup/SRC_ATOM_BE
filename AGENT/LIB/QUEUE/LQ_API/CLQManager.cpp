...
//		RTE_LOG(ERR, EAL, "%s\n", GetErrorMsg());
	...
	FetchMaria fdata;
	ret = m_pclsDbConn->Query(&fdata, pszQuery, strlen(pszQuery));
	if(ret < 0)
	{
		SetErrorMsg("Query Fail [%s] [%d:%s]", pszQuery, ret, m_pclsDbConn->GetErrorMsg(ret));
		delete m_pclsDbConn;
		return -E_Q_DB_FAIL;
	}
...
//								);
	...
	if( ((m_unReadRingCount + m_unWriteRingCount) == 0) && (a_nCmdType != DEF_CMD_TYPE_SEND) && (a_nCmdType != DEF_CMD_TYPE_ONLY) )
	{
 ...
 * \param a_nTime is Wait Time (sec)
 * \return 
 *   - 0 on Recieved Signal 
 *   - -1 Timed out
 *   - -E_Q_NOMEM Ring Index Over
 */
int CLQManager::ReadWait(int a_nTime)
{
	int ret = 0;
	//Set to Timeout Sec
	struct timespec tWait;
	tWait.tv_sec = a_nTime;
	tWait.tv_nsec = 1000000;
 ...
 * \param a_pszData is Data Pointer to Insert 
 * \param a_nSize is Size of Data
 * \param a_bSendSignal is Flag of To Send Signal, (if ture, Send Signal)
 * \return 
 *   - 0 on Success
 ...
 */
int CLQManager::SendCommand(char *a_szProcName, int a_nInstanceID, char *a_pszData, int a_nSize, bool a_bSendSignal)
{
	int ret = 0;
	struct rte_ring *pstRing = NULL;
	char pszQName[128];
 ...
 *   - -E_Q_SRCH No process has a PID matching pid
 */
int CLQManager::SendCommandResult(char *a_pszData, int a_nSize, bool a_bSendSignal)
{
	int ret = 0;
