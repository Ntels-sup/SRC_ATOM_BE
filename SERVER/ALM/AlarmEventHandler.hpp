...
public:
    ...
    bool            DecodingJSON();
    bool            DecodingJSON(ST_AlarmEventMsg & _stAlarmEventMsg,
                                 std::string      & _buffer);
...
private:
    ...
    bool            initAlarmConfMap(DB * _db);
    bool            getPkgName(std::vector<std::string> & _vec, DB * _db);
    bool            trySnmp(ST_AlarmStatus & _stAlarmStatus);
...
private:
    ...
    // KEY : pkg_name
    std::unordered_map <std::string, AlarmCodeConf>   map_alarm_conf_;
    SnmpTrap                snmp_;
