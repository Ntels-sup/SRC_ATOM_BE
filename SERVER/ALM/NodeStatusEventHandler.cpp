     ...
     AlarmEventHandler * _alarm_h)
{
    ...
    if(initAlarmDecisionMap(_db) == false)
    {
        gAlmLog->ERROR("%-24s| Init - fail",
            "NodeStatusEventHandler");
        return false;
    }
...
bool NodeStatusEventHandler::
initAlarmDecisionMap(DB * _db)
{
    map_alarm_decision_.clear();

    std::vector<std::pair<std::string, std::string> >    vec;

    if(getPkgNameAndNodeType(vec, _db) == false)
    {
        gAlmLog->ERROR("%-24s| initAlarmDecisionMap - can't find pkg names",
            "NodeStatusEventHandler");
    ...
    for(auto iter = vec.begin(); iter != vec.end(); ++iter)
    {
        ...
        {
            gAlmLog->ERROR("%-24s| initAlarmDecisionMap - can't init for alarm decision",
                "NodeStatusEventHandler");
            return false;
        }

        map_alarm_decision_[iter->first + ":" + iter->second] = alarm_decision;

        //gAlmLog->INFO("%-24s| initAlarmDecisionMap - alarm decision [%s] [%s]",
        //        "NodeStatusEventHandler",
        //        (iter->first).c_str(),
        //        (iter->second).c_str());
    }

    gAlmLog->INFO("%-24s| initAlarmDecisionMap - alarm decision map cnt [%d]",
        "NodeStatusEventHandler",
        map_alarm_decision_.size());
...
bool NodeStatusEventHandler::
getPkgNameAndNodeType(std::vector<std::pair<std::string,
                                            std::string> > & _vec,
                        DB * _db)
{
    char sql[128];
    ...
    {
         gAlmLog->WARNING("%-24s| getPkgNameAndNodeType -can't get pkg name and node type [%d:%s] [%s]",
            "NodeStatusEventHandler",
			ret,
            _db->GetErrorMsg(ret),
            sql);
...
{
    auto iter = map_alarm_decision_.find(_stAlarmEventMsg.pkg_name_ +
                                            ":" +
                                         _stAlarmEventMsg.node_type_);

    if(iter == map_alarm_decision_.end())
    {
    ...
    AlarmDecision & alarm_decision = iter->second;
