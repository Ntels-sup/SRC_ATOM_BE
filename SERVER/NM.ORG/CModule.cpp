...
bool CModule::Loading(string& a_strModule)
{
	...
	if (m_pHandle == NULL) {
		g_pclsLog->ERROR("MOD, dlopen failed");
		return false;
	}
...
void CModule::SetOption(ST_MODULE_OPTIONS& a_stOpt)
{
	m_stOption = a_stOpt;
	return;
}
...
bool CModule::Process(void)
{
    ...
    if (m_pstModuleOp->bThreadRun) {
		...
		if (curtime - m_tLastProcTime > 0) {		// 최소 1초에 한번 실행 
