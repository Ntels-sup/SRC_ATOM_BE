...
int insrtFunc(time_t stTm, time_t endTm)
{
		...
		fp = fopen("./TEST_QUERY3.sql","w");
		curTm = stTm;
		//fprintf(fp,"ALTER TABLE DIAM_STS DISABLE KEYS;\n");
		fprintf(fp,"INSERT INTO TOF_STAT_PARSER(PRC_DATE, NODE_NAME, DST_FLAG, PROC_NAME, CTF_NODE_NAME, RECORD_TYPE, INPUT_CNT, ERROR_CNT, OUTPUT_CNT, FILE_CNT)\n");
		fprintf(fp,"VALUES(FROM_UNIXTIME(%lu), \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", %d, %d, %d, %d)\n", curTm, "CSCF", "N", 
						"FPS","PGW01", "85", 1000, 0, 1000,1);
		//curTm += 300;
		...
		while(1){
				...
				fprintf(fp,", (FROM_UNIXTIME(%lu), \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", %d, %d, %d, %d)\n", curTm, "CSCF", "N", 
								"FPS","PGW01", "85", 1000, 0, 1000,1);
				curTm += 300;
		}
		fprintf(fp,";");
