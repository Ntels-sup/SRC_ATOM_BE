...
//    if(m_pDB->Connect("127.0.0.1", 3306, "atom", "atom", "ATOM")  != true)
    nDbRet = m_pDB->Connect(g_pcCFG.DB.m_strDbAddr.c_str(), g_pcCFG.DB.m_nDbPort, g_pcCFG.DB.m_strDbuserId.c_str(), g_pcCFG.DB.m_strDbPasswd.c_str(), g_pcCFG.DB.m_strDbName.c_str());
...
//  if(LoadCoworkNo("ATOM", "NM", "TRM", "WSM") == TRM_NOK)
    if(m_Process.LoadCoworkNo((char *)g_pcCFG.LOG.m_strNodeName.c_str(), (char *)g_pcCFG.LOG.m_strNmName.c_str(), (char *)g_pcCFG.LOG.m_strTrmName.c_str(), (char *)g_pcCFG.LOG.m_strWsmName.c_str(), m_stcoworkinfo) == TRM_NOK)
    {
...
//	g_pcLog->INFO("nRecvFlag[%d], nCommand [%d]", nRecvFlag, nCommand);
	switch(nRecvFlag)
	{
		case TRM_RECV_TRACE_ON:		// Trace On
...
// 	Debug - Web 답변 처리 위한 
					if(g_pcNM.SendWSMResponseMg(&stcoworkinfo, m_stTraceRequest, 1)  == TRM_NOK )
					{
...
//							nCommand = 1;
			...
			else if(nCommand == 1) // Response
			{
				...
				if(g_pcNM.GetResponseMsg(m_stTraceResponse) == TRM_NOK)				
				{
					...
					// send response msg to WSM
					if(g_pcNM.SendWSMResponseMsg(&stcoworkinfo, m_stTraceResponse, false)  == TRM_NOK )
					{
					    g_pcLog->INFO("SendWSMResponseMsg Error");
					    return TRM_NOK;
					}				    
				...
				else
				{
				    ...
				    if(g_pcNM.SendWSMResponseMsg(&stcoworkinfo, m_stTraceResponse, true)  == TRM_NOK )	
				    {
				        g_pcLog->INFO("SendWSMResponseMsg Error");
				        return TRM_NOK;
				    }
			...
			break;
		case TRM_RECV_TRACE_OFF:	// Trace Off
...
//		}
    ...
    // 3. Hist table에 정보 입력
//    m_TraceHist.InsertHist(&stTraceHist);
