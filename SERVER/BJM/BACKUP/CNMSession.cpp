	...
	for(i = 0; i < (int)m_vBatchRequest.size(); i++)
	{
		...
		if(m_vBatchRequest[i]->sequence == nResponseSeq)
		{
    		...
    		try {
				...
				if(strncmp(a_batchresponse->status, "failed", 6) == 0 || strncmp(a_batchresponse->status, "pending", 6) == 0)
				{
		    ...
		    if(m_BatchHist.HistUpdate(m_pDB, &stbatchhist) == false)
			{
