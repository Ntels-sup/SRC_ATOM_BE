...
CScheduler::CScheduler()
{
	...
	m_nBatchJobCount[BJM_MAX_BATCH_JOB] = { 0 };
	m_nBatchFlowCount = 0;
	m_nProcessCount = 0;
	m_nMultiJobCount = 0;
	...
	switch(nRecvFlag)
	{
		...
		case BJM_RECV_START:
			if(g_pcNM.IsRequest() == 1 || g_pcNM.IsRequest() == 2) // response, notify
			{
				...
				{
					m_stBatchResponse->exit_cd = 22;	// for debug 
					...
					{
						g_pcNM.SendWSMResponseMsg(1, &stcoworkinfo, m_stBatchRequest, m_stBatchResponse);
					}
				}
			}
		 	break;
		case BJM_RECV_REGIST:
			g_pcLog->DEBUG("Receive Regist [%d]", nRecvFlag);
			break;
		case BJM_RECV_ERROR:
...
//	if(strcmp(m_vMultiFlow[i]->group_name, a_strGroupName) == 0)
	{
	    for (i = 0; i < (int)m_vMultiFlow.size(); i++)
	    {
    	    ...
    	    if(strcmp(m_vMultiFlow[i]->job_name, a_strJobName) == 0 && m_vMultiFlow[i]->exit_cd == a_nExitCd)
        	{
...
int CScheduler::CheckCurrentTime()
{
	...
	while(1)	
	{
		...
		{
			for(nJobCount = 0; nJobCount < m_nBatchJobCount[nGroupCount]; nJobCount++)
			{
				RunSchedule(strCurrent_date, nGroupCount, nJobCount);
			}
		}
...
void CScheduler::RunSchedule(char *a_strCurrent_date, int a_nGroupCount, int a_nJobCount)
{
	...
	{
		if(strncmp(m_vBatchJob[a_nJobCount]->nextExecDate, a_strCurrent_date, 14) == 0 && m_vProcess[a_nJobCount] != NULL)
		{
			...
			if( (strncmp(m_vBatchJob[a_nJobCount]->rootjob_yn, "Y", sizeof("Y")) == 0) && 		
					(strncmp(m_vBatchJob[a_nJobCount]->run_yn, "Y", sizeof("Y")) == 0))
			{
				if(SetBatchHist(m_vBatchJob[a_nJobCount], m_vProcess[a_nJobCount], a_nJobCount) == BJM_NOK )
	    	    {
				...
				else
				{
					g_pcLog->INFO("%s,\t%s,\tCurrent : %s, nextExecDate[%d] : %s", 
				      				m_vBatchJob[a_nJobCount]->group_name,  
									m_vBatchJob[a_nJobCount]->job_name,
									a_strCurrent_date, 
									a_nJobCount, 
									m_vBatchJob[a_nJobCount]->nextExecDate);
				}
			}
			m_nDBInitFlag = 0;
		}
...
int CScheduler::GetJobInfo(int a_nGrpCount)
{
	int j = 0;
	...
	m_vBatchGroup.push_back(&m_stbatchgroups[a_nGrpCount]);

	m_nBatchJobCount[a_nGrpCount] = m_BatchJob.LoadJobInfo(&m_stbatchgroups[a_nGrpCount], m_stbatchjobs);
	if(m_nBatchJobCount[a_nGrpCount] < 0)
	{
	...
	else
	{
		if(m_nBatchJobCount[a_nGrpCount] != 0) 
		{
			for(j = 0; j < m_nBatchJobCount[a_nGrpCount]; j++)
			{
				...
				g_pcLog->INFO("group_name[%d] : %s, job_name[%d] : %s", 
								a_nGrpCount, &m_stbatchgroups[a_nGrpCount].group_name, j, &m_stbatchjobs[j].job_name);
		...
		g_pcLog->DEBUG("m_nBatchJobCount[%d] : %d", a_nGrpCount, m_nBatchJobCount[a_nGrpCount]);
	}
	return BJM_OK;
}
int CScheduler::GetFlowInfo()
{
	...
	else
	{
		if(m_nMultiJobCount != 0)
		{
			...
			{
				strcpy(m_stMultiJobFlow[j].group_name, m_BatchGroup.GetGroupName(m_stMultiJobFlow[j].job_name));
