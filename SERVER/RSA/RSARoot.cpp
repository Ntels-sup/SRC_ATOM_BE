...
int RSARoot::SendResponseToRSA(CSocket *a_pclsClient, int a_nCmd, const char *a_szBody)
{
	...
	m_pclsSock->SetPayload(a_szBody);
...
int RSARoot::CheckTrapMsg(std::string &a_strJson)
{
	...
	if(SendResponseToRSA(m_pclsClient, CMD_RSA_ROOT_TRAP, doc.str().c_str()) < 0)
	{
...
//		m_pclsSock->SetPayload(strBuff);
		...
		if(SendResponseToRSA(client, nCmd, strBuff.c_str()) < 0)
		{
			m_pclsSock->ClosePeer(client);
			continue;
		}
