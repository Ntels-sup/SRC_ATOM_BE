...
//	stOption->m_pclsAddress;    // Agent 내의 모듈, Process ID 검색
	...
	p_clsLog->DEBUG("Package Name : %s", stOption->m_szPkgName);
	p_clsLog->DEBUG("Node Type : %s", stOption->m_szNodeType);
	p_clsLog->DEBUG("Node ID : %d", stOption->m_nNodeNo);
	p_clsLog->DEBUG("Proc ID : %d", stOption->m_nProcNo);
