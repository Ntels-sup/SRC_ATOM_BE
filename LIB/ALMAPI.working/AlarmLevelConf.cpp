...
bool AlarmLevelConf::
SetAlarmLevel(DBConfig   * _dbConfig,
              ...
              const char * _node_type)
{
    ...
    int eNo = 0;
    if(_dbConfig->Get(f, eNo, sql) == false)
    {
        gAlmLog->ERROR("%-24s| SetAlarmLevel - get fail [%d:%s] [%s]",
            "AlarmLevelConf",
            eNo,
    ...
    while(f.Fetch())
    {
        ...
        if(setSeverityByMonitor(_dbConfig,
                                def.monitor_id_,
                                def.list_) != true)
        {
            prn();
...
bool AlarmLevelConf::
setSeverityByMonitor(DBConfig            * _dbConfig,
                     int                   _monitor_id,
                     std::list<ST_Level> & _list)
{
    ...
    if(_dbConfig->Get(f, eNo, sql) == false)
    {
        gAlmLog->ERROR("%-24s| setSeverityByMonitor - get fail [%d:%s] [%s]",
            "AlarmLevelConf",
            eNo,
            _dbConfig->GetErrorMsg(eNo),
            sql);
                 ...
                 std::string & _target)
{
    ...
    gAlmLog->DEBUG("------ [%s:%d] CODE[%s] size[%d] map size [%d]",
            __FILE__,
            __LINE__,
            _code.c_str(),
            _code.size(),
