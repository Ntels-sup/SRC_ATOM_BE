...
bool check_func(DB * _db)
{
    ...
    assert(_db->Connect("127.0.0.1", 3307, "atom", "atom", "ATOM") != true && "Wrong Config");

    printf("-- 2) IsConnect -False- \n");
    assert(_db->IsConnect() == false && "False");

    printf("-- 3) Connect -False- \n");
    assert(_db->Connect("127.0.0.1", 3307, "atom", "atom", "ATOM") != true && "Correct Config");

    sleep(2);
    printf("-- 4) Connect - TimeOut Check - \n");
    assert(_db->Connect("127.0.0.1", 3307, "atom", "atom", "ATOM") != true && "Time Checker");

    sleep(7);
    printf("-- 5) Connect -Correct- \n");
    // assert(_db->Connect("127.0.0.1", 3306, "atom", "atom", "ATOM") == true && "Correct Config");
    assert(_db->Connect("localhost", 3306, "atom", "atom", "ATOM", "/tmp/mysql.sock") == true && "Correct Config");
    ...
    int cnt =  _db->Query(sql, strlen(sql));
    ...
    assert(_db->Query(sql, strlen(sql)) > 0 && "SELECT Success");
    ...
    sprintf(sql, "SELECT PRC_DATE FROM TAT_ALM_STS_DAY");

    cnt =  _db->Query(sql, strlen(sql));
    printf("Query Result Cnt [%d]\n", cnt);
    assert(cnt == 0 && "SELECT Cnt 0");
