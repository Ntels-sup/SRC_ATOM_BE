...
bool AlarmDecision::
Init(DB * _db,  ST_Profile & _profile)
{
    return Init(_db, _profile.m_strPkgName, _profile.m_strNodeType);
}

bool AlarmDecision::
Init(DB * _db,  std::string & _pkg_name, std::string & _node_type)
{
    if(code_conf_.SetAlarmCode(_db,
    ...
    if(level_conf_.SetAlarmLevel(_db,
                                 _pkg_name.c_str(),
...
Do(ST_AlarmStatus & _stAlarmStatus)
{
    ...
    if(def == NULL)
    {
        gAlmLog->INFO("%-24s| Do2 - Unknown defined Alarm code [%s]",
            "AlarmDecision",
            _stAlarmStatus.code_.c_str());
        return false;
    }
                            ...
                            _stAlarmStatus.target_) == true)
    {
        ...
        gAlmLog->DEBUG("%-24s| Do2 - Code[%s] Value[%s] Severity[%d]",
            "AlarmDecision",
            _stAlarmStatus.code_.c_str(),
    ...
    /*--
    gAlmLog->DEBUG("%-24s| ------ Do2 - level conf is not exist node type [%s] code [%s] target [%s]",
            "AlarmDecision",
            _stAlarmStatus.node_type_.c_str(),
            _stAlarmStatus.code_.c_str(),
            _stAlarmStatus.target_.c_str());
