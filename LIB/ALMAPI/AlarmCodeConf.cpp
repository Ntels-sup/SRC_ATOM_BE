            ...
            const char      * _pkg_name)
{
    ...
    int cnt = queryAlarmCode(_db, f, _code.c_str(), _pkg_name);
    ...
    if(cnt != 1)
    {
        gAlmLog->WARNING("%-24s| SetAlarmCode - select too many data [%s:%s]",
            "AlarmCodeConf",
            _code.c_str(),
            _pkg_name);
             ...
             const char *    _pkg_name)
{
    ...
    int cnt = queryAlarmCode(_db, f, "", _pkg_name);
               ...
               const char * _pkg_name)
{
    ...
    int     ret = 0;
    if((ret = _db->Query(&_f, sql, strlen(sql))) < 0)
    {
        gAlmLog->WARNING("%-24s| SetAlarmCodeDef - select fail [%d:%s] [%s]",
            "AlarmCodeConf",
			ret,
            _db->GetErrorMsg(ret),
            sql);
...
bool AlarmCodeConf::
setAlarmCodeDef(FetchMaria & _f)
{
    if(_f.Fetch() == false)
    {
        gAlmLog->WARNING("%-24s| SetAlarmCodeDef - fetch fail",
            "AlarmCodeConf");
        return false;
    }
    ...
    /*--
    gAlmLog->DEBUG("%-24s| P[%s], CODE[%s] T[%d] G[%d] A-CODE[%s] CAUSE[%.16s] SEVER[%d] SMS[%s] EMAIL[%s] SNMP[%s]",
        "AlarmCodeConf",
        st.pkg_name_.c_str(),
        st.code_.c_str(),
        st.event_type_id_,
