...
CMesgExchSocket::CMesgExchSocket() : CSocket(CSocket::IP_TCP)
{
...
CMesgExchSocket::CMesgExchSocket(int a_nSocket) : CSocket(a_nSocket)
{
	m_vecBuff.reserve(CProtocol::MAX_LENGTH);
	memset(m_szTempBuff, 0x00, sizeof(m_szTempBuff));
...
CMesgExchSocket::~CMesgExchSocket()
{
	//NodeOP
	return;
}

bool CMesgExchSocket::SendMesg(void)
{
	m_strErrorMsg.clear();

	GenStream(m_vecBuff);

	size_t sn = Send((char*)&m_vecBuff[0], m_vecBuff.size());
	if (sn != m_vecBuff.size()) {
		m_strErrorMsg = CSocket::m_strErrorMsg;
		m_vecBuff.clear();
		return false;
	}
	m_vecBuff.clear();
...
bool CMesgExchSocket::SendMesg(CProtocol& a_clsProto)
{
	m_strErrorMsg.clear();

	a_clsProto.GenStream(m_vecBuff);
...
int CMesgExchSocket::RecvMesg(CProtocol* a_pcProto, int a_nWaitTime)
{
	m_strErrorMsg.clear();
	...
	// receive header
	if (readn > 0) {
		readn = Recv(m_szTempBuff, readn, a_nWaitTime);
		if (readn <= 0) {
			if (CSocket::m_enError == CSocket::SOCK_TIMEOUT) {
				return 0;
			}
			m_vecBuff.clear();
			m_strErrorMsg = CSocket::m_strErrorMsg;
	...
	// receive body
	int blen = Recv(m_szTempBuff, readn, a_nWaitTime);
	if (blen <= 0) {
		if (CSocket::m_enError == CSocket::SOCK_TIMEOUT) {
			return 0;
		}
